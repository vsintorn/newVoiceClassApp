...
processFolder_3 <- function(folderName,length) {
...
meeting2csv <- function(fileNameWave){
  ...
  meeting <- processFolder_3(outputFolder,10)
  ...
  nameCSV <- paste(shortName, '.csv', sep="")
  
  setwd('../..')
  # Write out csv dataset.
  write.csv(data, file = 'recordingSilenced.csv', row.names=F)
  
  return('recordingSilenced.csv')
}
